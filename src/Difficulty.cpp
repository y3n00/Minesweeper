...
constexpr Difficulty::Difficulty(const char* name, uint8_t w, uint8_t h, uint8_t mines)
...
const Difficulty* Difficulty::draw_slider(Rectangle rect) {
    ...
    const auto slider_max = Constants::Difficulty::segment_size * diffs.size() - 1;

    static float slider_value = 0.0f;
    GuiSlider(rect, "Select difficulty", NULL, &slider_value, 0, slider_max);
    auto&& diff = diffs[static_cast<size_t>(slider_value / Constants::Difficulty::segment_size)];
